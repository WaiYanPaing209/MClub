...
router.get("/", async (req, res) => {
  ...
  const arr = await db.selectMany("agent", { master: req.session.agentCode, role: "master" });

  res.render("master", { arr, session, agent });
});
...
router.post("/", async (req, res) => {
  ...
  if (body.req == "new") {
    ...
    await db.updateOne(
      "agent_users",
      { agentCode, username: agentCode, role: "master" },
      { password: body.password }
    );
    ...
    if (agent.balance >= amount) {
      ...
      await db.insertOne("master_transfer_log", {
