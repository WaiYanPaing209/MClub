...
router.get("/", async (req, res) => {
  ...
  const [arr, total, lastPage] = await db.selectPagination("datasheet", filter, page, pageLimit);
  const role = req.session.role;
  const [previous, next] = paginationURL(req.query, lastPage);
  res.render("master_daily", { arr, role, page, previous, next, req });
});
