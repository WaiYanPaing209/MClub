...
class Game {
  ...
  constructor(levelConfig, config = null) {
    ...
    this.botMode = Game.BotMode.sustain;
    this.rooms = [];
    this.botIndex = 0;
    ...
    this.onRefillBot = (nickname, profile) => {
      console.warn("Refill bot not implement yet!");
    };
  }
  ...
  newBot(level) {
    if (this.botPool.length > 0) {
      ...
      const balance = Math.floor(
        this.levelConfig[level].minAmount * (Math.random() + 2)
      );
  ...
  update() {
    ...
    for (const room of this.rooms) {
      if (this.autoBot) {
        if (
          room.totalBot() < this.botPerRoom &&
          room.totalAllPlayer() < this.totalPlayer &&
